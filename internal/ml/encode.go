...
func EncodeBoardTensor(b *game.Board, me game.CellState) [TensorLen]float32 {
	var t [TensorLen]float32
	for q := -4; q <= 4; q++ {
		for r := -4; r <= 4; r++ {
			c := game.HexCoord{Q: q, R: r}
			x, y := q+4, r+4 // 0..8
...
func AxialToIndex(c game.HexCoord) int { return (c.R+4)*GridSize + (c.Q + 4) }
